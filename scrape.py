...
Fetches the public arammayhem.com pages and writes the fresh champion + augment
data DIRECTLY into index.html, between the /*DATA_START*/ and /*DATA_END*/ markers.
No sub-folders, no separate data files.

Runs server-side (e.g. GitHub Actions) so there is no CORS restriction.
"""
import json, re, sys, time, datetime, os
import requests
...
def main():
    ...
    meta = {"updated": datetime.date.today().isoformat(), "patch": patch or "n/a",
            "source": "arammayhem.com", "champions": len(champs), "augments": len(augs)}
