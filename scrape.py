...
def main():
    ...
    tiers = current_tiers()   # preserve browser-harvested per-champion augment tiers
    meta = {"updated": datetime.date.today().isoformat(), "patch": patch or "n/a",
            "source": "arammayhem.com", "champions": len(champs),
            "augments": len(augs), "builds": len(builds), "tiers": len(tiers)}
...
def current_tiers():
    """Preserve the AUG_TIERS object already embedded in index.html (browser-harvested),
    so an automated run never wipes the per-champion augment tiers."""
    try:
        html = open(INDEX, encoding="utf-8").read()
        m = re.search(r"const AUG_TIERS\s*=\s*(\{.*?\});", html, re.S)
        if m:
            return json.loads(m.group(1))
    except Exception as e:
        print("  (could not read existing tiers:", e, ")")
    return {}
